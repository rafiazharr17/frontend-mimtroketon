...
export default function AddFasilitas() {
  ...
  const handleSubmit = async (e) => {
    ...
    setIsLoading(true);
    try {
      await api.post("/admin/fasilitas", formData);
      Swal.fire({
        icon: "success",
        title: "Berhasil!",
        text: "Barang ditambahkan ke inventaris.",
        customClass: { popup: "rounded-3xl" },
      });
  ...
  const handleChange = (e) => {
    ...
    // Hilangkan error otomatis saat ngetik ulang
      ...
      <div className="mb-8 flex items-center gap-4 w-full">
        ...
        <div>
          ...
          <p className="text-slate-500 mt-1">
            Catat sarana dan prasarana baru ke dalam sistem.
          </p>
        </div>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-8 relative z-10">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="lg:col-span-2">
              ...
              <input
                ...
                className={`w-full px-5 py-4 bg-white/50 border rounded-2xl outline-none transition-all font-medium text-slate-700 placeholder-slate-400 focus:ring-4 ${errors.nama_fasilitas ? "border-red-500 focus:ring-red-500/10 focus:border-red-500 bg-red-50/50" : "border-slate-200 focus:ring-cyan-500/10 focus:border-cyan-500"}`}
                placeholder="Contoh: Proyektor Epson EB-X05, Meja Belajar Kayu"
              />
            ...
            <div>
              ...
              <select
                ...
                className="w-full px-5 py-4 bg-white/50 border border-slate-200 rounded-2xl focus:ring-4 focus:ring-cyan-500/10 focus:border-cyan-500 outline-none transition-all font-medium text-slate-700 cursor-pointer">
                <option value="Sarana">
                  Sarana (Alat Pembelajaran/Peraga)
                </option>
                <option value="Prasarana">Prasarana (Ruang/Bangunan)</option>
                <option value="Elektronik">Alat Elektronik / IT</option>
            ...
            <div className="lg:col-span-2">
              <label className="block text-sm font-bold text-slate-700 mb-2">
                Keterangan / Catatan Lokasi (Opsional)
              </label>
              <textarea
                name="keterangan"
                rows="3"
                ...
                placeholder="Misal: Ditempatkan di Ruang Kelas 1-A"></textarea>
            </div>
          </div>
          ...
          <div className="flex justify-end gap-4 pt-8 border-t border-slate-100">
            ...
            <button
              type="submit"
              disabled={isLoading}
              className={`px-10 py-3.5 bg-gradient-to-r from-cyan-500 to-sky-500 hover:from-cyan-600 hover:to-sky-600 text-white font-bold rounded-xl shadow-lg shadow-cyan-500/30 transition-transform transform hover:-translate-y-0.5 ${isLoading ? "opacity-70" : ""}`}>
