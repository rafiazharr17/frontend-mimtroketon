...
export default function AddMapel() {
  ...
  const [listMapel, setListMapel] = useState([]); // State untuk cek mapel duplikat
  const [errorMsg, setErrorMsg] = useState(""); // Error per kolom
  ...
  useEffect(() => {
    // Ambil data mapel yang sudah ada untuk mencegah duplikasi
    api
      .get("/admin/mapel")
      .then((res) => setListMapel(res.data))
  ...
  const handleSubmit = async (e) => {
    ...
    // VALIDASI DUPLIKASI MAPEL
    const isDuplicate = listMapel.some(
      (m) => m.nama_pelajaran.toLowerCase() === namaTrimmed.toLowerCase(),
    );
    if (isDuplicate) {
    ...
    setIsLoading(true);
    try {
      await api.post("/admin/mapel", { nama_pelajaran: namaTrimmed });
      Toast.fire({
        icon: "success",
  ...
  const handleChange = (e) => {
    ...
    if (errorMsg) setErrorMsg(""); // Hilangkan error saat ngetik ulang
  };
      ...
      <div className="mb-8 flex items-center gap-4 w-full">
        ...
        <div>
          ...
          <p className="text-slate-500 mt-1">
            Daftarkan pelajaran baru ke sistem.
          </p>
        </div>
      </div>
