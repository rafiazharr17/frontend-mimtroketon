...
export default function HomeMapel() {
  ...
  useEffect(() => {
    api.get('/admin/mapel').then(res => setMapel(res.data.sort((a, b) => a.nama_pelajaran.localeCompare(b.nama_pelajaran)))).finally(() => setIsLoading(false));
  }, []);

  const filteredMapel = mapel.filter(m => m.nama_pelajaran.toLowerCase().includes(searchTerm.toLowerCase()));
  ...
  const handleExportExcel = () => {
    if(filteredMapel.length === 0) return Swal.fire('Kosong', 'Tidak ada data untuk diekspor', 'info');
    const data = filteredMapel.map((m, i) => ({"No": i + 1, "Nama Mata Pelajaran": m.nama_pelajaran}));
    exportToExcel(data, `Data_Mapel_${getFormattedTime()}`, "LAPORAN DAFTAR MATA PELAJARAN MI MUHAMMADIYAH TROKETON", exportTahun, exportSemester);
    setIsExportModalOpen(false);
  };

  const handleExportPDF = () => {
    if(filteredMapel.length === 0) return Swal.fire('Kosong', 'Tidak ada data untuk diekspor', 'info');
    const headers = ["No", "Nama Mata Pelajaran"];
    const data = filteredMapel.map((m, i) => [i + 1, m.nama_pelajaran]);
    exportToPDF(headers, data, `Data_Mapel_${getFormattedTime()}`, "LAPORAN DAFTAR MATA PELAJARAN MI MUHAMMADIYAH TROKETON", 'p', exportTahun, exportSemester);
    setIsExportModalOpen(false);
  };
  ...
  return (
    <AdminLayout>
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4 w-full">
        <div>
          <h1 className="text-3xl font-extrabold text-slate-800 tracking-tight">Kelola Mata Pelajaran</h1>
          <p className="text-slate-500 mt-1">Daftar mata pelajaran yang diajarkan di sekolah.</p>
        </div>
        <div className="flex gap-3 w-full sm:w-auto">
          ...
          <Link to="/admin/mapel/restore" className="flex items-center justify-center gap-2 px-5 py-2.5 bg-white hover:bg-red-50 border border-slate-200 hover:border-red-200 text-slate-700 hover:text-red-600 font-bold rounded-xl shadow-sm transition-all flex-1 sm:flex-none group">
            <svg className="w-5 h-5 text-slate-400 group-hover:text-red-500 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
            Recycle Bin
          </Link>
          <Link to="/admin/mapel/add" className="flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-rose-500 to-pink-600 text-white font-bold rounded-xl shadow-lg transform hover:-translate-y-0.5">+ Tambah</Link>
        </div>
      </div>

      <div className="mb-6 bg-white/80 backdrop-blur-xl border border-white shadow-xl shadow-slate-200/40 rounded-3xl p-5">
        <input type="text" placeholder="Cari Nama Mapel..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full px-5 py-3 bg-white border border-slate-200 rounded-xl focus:border-rose-500 outline-none text-slate-700" />
      </div>
          ...
          <table className="w-full text-left border-collapse min-w-max">
            <thead>
              <tr className="bg-slate-50/80 border-b border-slate-200 text-slate-500 text-sm uppercase tracking-wider">
                <th className="px-8 py-5 font-bold">Nama Mata Pelajaran</th><th className="px-8 py-5 font-bold text-center">Aksi</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {isLoading ? ( <tr><td colSpan="2" className="px-8 py-10 text-center text-slate-400">Memuat...</td></tr>
              ) : filteredMapel.length === 0 ? ( <tr><td colSpan="2" className="px-8 py-10 text-center text-slate-400">Kosong.</td></tr>
              ) : (
                filteredMapel.map(m => (
                  <tr key={m.id} className="hover:bg-rose-50/30 transition-colors">
                    <td className="px-8 py-5 font-bold text-slate-800">{m.nama_pelajaran}</td>
                    <td className="px-8 py-5 text-center w-48"><Link to={`/admin/mapel/detail/${m.id}`} className="px-5 py-2 bg-white border border-slate-200 hover:border-rose-400 text-sm font-bold rounded-xl transition-all">Detail</Link></td>
                  </tr>
                ))
              )}
            </tbody>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm overflow-hidden transform transition-all scale-100 animate-slide-up">
            <div className="px-6 py-5 border-b border-slate-100 flex justify-between items-center bg-slate-50/80">
              <h3 className="text-lg font-black text-slate-800">Cetak Data Mapel</h3>
